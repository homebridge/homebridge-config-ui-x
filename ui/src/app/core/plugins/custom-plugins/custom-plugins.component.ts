...
export class CustomPluginsComponent implements OnInit, OnDestroy {
  ...
  private updateConfigBlocks(pluginConfig: Record<string, unknown>[]): void {
    ...
    // Update the property with the modified config
   ...
   */
  private schemaFormUpdated(updatedData: unknown): void {
    ...
    // Update the first item in the config array with the new data
    const currentConfig = this.pluginConfig
    if (currentConfig.length > 0 && typeof updatedData === 'object' && updatedData !== null && !Array.isArray(updatedData)) {
      const updatedConfig = [...currentConfig]
      updatedConfig[0] = updatedData as Record<string, unknown>
      this.pluginConfig = updatedConfig
    }
