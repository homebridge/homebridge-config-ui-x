...
export class ManageVersionComponent implements OnInit {
  ...
  private async lookupVersions(): Promise<void> {
    ...
    try {
      ...
      if (currentPlugin?.installedVersion && !newVersions.some(x => x.version === currentPlugin.installedVersion)) {
        newVersions.push({
      ...
      if (!newVersions.some(x => x.version === this.versionSelect) && result.tags.latest) {
