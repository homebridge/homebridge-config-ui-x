...
import { HB_V2_MODAL_DATA, MANAGE_PLUGIN_MODAL_DATA, PLUGIN_LOGS_MODAL_DATA } from '@/app/core/modal-data-tokens'
...
export class ManagePluginComponent implements OnInit, OnDestroy {
  ...
  public ngOnInit(): void {
    ...
    this.targetVersionPretty.set(
      targetVer === 'latest'
        ? `v${latestVer}`
        : (/^\d/.test(targetVer) ? `v${targetVer}` : targetVer),
    )
        ...
        if (this.pluginName === 'homebridge-config-ui-x') {
          try {
            if (this.$settings.webroot && this.$settings.webroot !== '/') {
              ...
              await this.$api.put('/server/webroot', {
            ...
            console.error('Failed to update webroot or set restart flag:', error)
            window.location.href = 'restart'
          }
        } else {
          try {
