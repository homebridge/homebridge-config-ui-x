...
import { UpdateAllExclusionReason, UpdateAllItemStatus, UpdateAllJournal, UpdateAllJournalItem, UpdateAllPlan, UpdateAllSnapshot } from '@/app/core/update-all/update-all.interfaces'
...
@Component({
  ...
  imports: [TranslatePipe, UpdateAllItemRowComponent],
  standalone: true,
...
export class UpdateAllModalComponent implements OnInit, OnDestroy {
  ...
  private pendingEvents: Array<{ name: string, status: UpdateAllItemStatus }> = []
  ...
  private async subscribeToRun(): Promise<void> {
    try {
      ...
      if (this.runId && snapshot.journal && snapshot.journal.runId !== this.runId) {
        return
      }
      if (snapshot.journal) {
        this.runId = snapshot.journal.runId
      ...
      for (const event of this.pendingEvents) {
        this.setItemStatus(event.name, event.status)
      }
  ...
  private attachSocketListeners(): void {
    ...
    const applyOrBuffer = (name: string, status: UpdateAllItemStatus) => {
      if (this.awaitingSnapshot) {
        this.pendingEvents.push({ name, status })
        return
      }
      this.setItemStatus(name, status)
    }
  ...
  private async showSummary(handOver: boolean): Promise<void> {
    ...
    if (handOver && (homebridgeRestarting || uiRestarting)) {
      ...
      this.$activeModal.close('handover')
  ...
  private watchChildBridges(usernames: string[]): void {
    ...
    on('child-bridge-status-update', (data: { username?: string, status?: string }) => {
      ...
      if (data.status === 'ok' && !this.restartedBridges().includes(data.username)) {
        const done = [...this.restartedBridges(), data.username]
        this.restartedBridges.set(done)
        // Every bridge is back - the give-up timer has nothing left to do
        if (this.childRestartTimer && usernames.every(x => done.includes(x))) {
          clearTimeout(this.childRestartTimer)
