...
describe('update all', () => {
  ...
  describe('the plan modal', () => {
    ...
    // The icon fallback belongs to UpdateAllItemRowComponent now - one owner
    // for every list in this modal, including the skipped section
    ...
    describe('the restart plan it previews', () => {
      ...
      it('flags the ui restart independently of the homebridge one', async () => {
        ...
        expect(component.restartPlan()).toEqual({ ui: true, homebridge: false, childBridgeCount: 0 })
      })
  ...
  describe('the progress modal', () => {
    ...
    async function open(options: {
      ...
      // The run is entered the only way the app can now: plan, confirm, start.
      ...
      fixture.detectChanges()
      await settle()
      // Confirm the plan - the only door into the run since the auto-reopen
      // (the old `resume` mode) was removed
    ...
    it('ignores a reconnect snapshot for a different run after settling', async () => {
      ...
      io.socket.respondTo('subscribe', { active: true, journal: makeJournal({ runId: 'run-2' }) })
      io.markConnected()
      await settle()

      expect(component.phase()).toBe('summary')
      expect(component.journal()?.runId).toBe('run-1')
    })
    ...
    it('does not restart the child-bridge watch on a reconnect', async () => {
      ...
      const watchCalls = () => ws.connectToNamespace.mock.calls.filter(call => call[0] === 'child-bridges').length
