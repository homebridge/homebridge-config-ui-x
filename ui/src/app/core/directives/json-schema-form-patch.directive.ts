...
export class JsonSchemaFormPatchDirective implements AfterViewInit, OnDestroy {
  ...
  private fieldsetSyncRaf = new WeakMap<HTMLElement, number>()
  ...
  public ngAfterViewInit(): void {
    ...
    this.observer = new MutationObserver((mutations) => {
      if (this.patchingInProgress) return
  ...
  private scheduleFieldsetSync(
    fieldset: HTMLElement,
    proxy: HTMLButtonElement,
    realToggle: HTMLElement | null,
    collapseBody: HTMLElement | null,
  ) {
    const existing = this.fieldsetSyncRaf.get(fieldset)
    if (typeof existing === 'number') {
      cancelAnimationFrame(existing)
    }

    const raf = requestAnimationFrame(() => {
      this.fieldsetSyncRaf.delete(fieldset)
      this.syncProxyExpanded(fieldset, proxy, realToggle, collapseBody)
    })

    this.fieldsetSyncRaf.set(fieldset, raf)
  }
  ...
  private getExpandedState(fieldset: HTMLElement, collapseBody: HTMLElement | null, realToggle: HTMLElement | null) {
    ...
    if (toggleExpanded === 'true') return true
    if (toggleExpanded === 'false') return false

    if (realToggle?.classList.contains('collapsed')) return false
  ...
  private isFieldsetExpanded(fieldset: HTMLElement) {
    ...
    for (const el of Array.from(controls)) {
      ...
      if (this.isVisiblyRendered(h)) {
        if (h.tagName.toLowerCase() === 'legend') continue
        if (h.classList.contains('jsf-fieldset-proxy')) continue
        return true
      }
    }
    return false
    ...
    inputs.forEach((el) => {
      ...
      if (labelWrap && labelWrap.contains(control)) {
        labelWrap.querySelectorAll<HTMLElement>('*').forEach((node) => {
          ...
          if (node.contains(control)) return
          if (node.hasAttribute('data-jsf-a11y-hidden')) return
      ...
      if (parent) {
        Array.from(parent.children).forEach((sibling) => {
          ...
          if (sibling === control) return
          if (labelEl && sibling === labelEl) return
          if (sibling.hasAttribute('data-jsf-a11y-hidden')) return
          if (this.isInteractiveElement(sibling)) return
