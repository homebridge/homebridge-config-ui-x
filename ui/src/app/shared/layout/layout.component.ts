...
import { UPDATE_ALL_MODAL_DATA, UpdateAllJournal } from '@/app/core/update-all/update-all.interfaces'
  ...
  private async checkUpdateAllState(): Promise<void> {
    ...
    try {
      ...
      if (!journal.finishedAt || (finishedRecently && !journal.acknowledged)) {
        // `resume` skips the plan phase - this run already exists, we are
        // reattaching to it (or showing its summary) after the UI restarted
