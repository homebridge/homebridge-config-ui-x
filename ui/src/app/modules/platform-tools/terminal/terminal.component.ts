...
export class TerminalComponent implements OnInit, OnDestroy {
  ...
  public ngOnInit() {
    console.debug('[Component] ngOnInit called, isTerminalReady:', this.$terminal.isTerminalReady());
    ...
    if (this.$terminal.isTerminalReady()) {
      console.debug('[Component] Terminal ready, using reconnectTerminal for proper session management');
    ...
    console.debug('[Component] ngOnInit - persistence:', this.$settings.terminalPersistence, 'hasActiveSession:', this.$terminal.hasActiveSession());

    // Start or reconnect to the terminal
    if (this.$settings.terminalPersistence && this.$terminal.hasActiveSession()) {
      console.debug('[Component] Calling reconnectTerminal');
      this.$terminal.reconnectTerminal(this.termTarget(), {}, this.resizeEvent);
    } else {
      console.debug('[Component] Calling startTerminal');
