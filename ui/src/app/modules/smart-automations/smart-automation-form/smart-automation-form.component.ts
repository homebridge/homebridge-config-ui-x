...
export class SmartAutomationFormComponent {
  ...
  public getAutomationDescription(type: SmartAutomation['type'] | undefined): string {
    if (type === 'smart-light-group') {
      return 'Publishes a switch that stores the selected lights’ On/Off state when turned on, then restores that state when turned off. Designed for use with Siri or other voice assistants; it does not work through direct control in the Home app.'
    }
