...
export class SettingsComponent implements OnInit {
  ...
  public uiAlwaysShowBetasIsSaving = signal(false)
  ...
  public uiWebrootIsSaving = signal(false)
  ...
  private async initialize(): Promise<void> {
    ...
    this.uiAlwaysShowBetasFormControl.patchValue(this.$settings.env.plugins?.alwaysShowBetas || false, { emitEvent: false })
    ...
    this.uiWebrootFormControl.patchValue(this.$settings.webroot || '', { emitEvent: false })
  ...
  private async uiAlwaysShowBetasSave(value: boolean): Promise<void> {
    try {
      this.uiAlwaysShowBetasIsSaving.set(true)
      this.$settings.setEnvItem('plugins.alwaysShowBetas', value)
      await this.saveUiSettingChange('plugins.alwaysShowBetas', value)
      setTimeout(() => {
        this.uiAlwaysShowBetasIsSaving.set(false)
      }, 1000)
    } catch (error) {
      console.error(error)
      this.$toastr.error(error.message, this.$translate.instant('toast.title_error'))
  ...
  private async uiWebrootSave(value: string): Promise<void> {
    try {
      this.uiWebrootIsSaving.set(true)
      ...
      setTimeout(() => {
        this.uiWebrootIsSaving.set(false)
        this.showRestartToast()
      }, 1000)
    } catch (error) {
      console.error(error)
