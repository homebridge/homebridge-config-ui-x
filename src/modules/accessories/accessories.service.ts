...
export class AccessoriesService {
  ...
  public async connect(client: Socket) {
    ...
    const loadAllAccessories = async (refresh: boolean) => {
      ...
      client.emit('accessories-ready-for-control') // Keep for backward compatibility
  ...
  private async getMatterAccessory(uniqueId: string): Promise<MatterService> {
    try {
      // Parse uniqueId: matter:{uuid} or matter:{uuid}:{partId}
      const parts = uniqueId.replace('matter:', '').split(':')
      const uuid = parts[0]

      // Request detailed info via IPC
      this.homebridgeIpcService.sendMessage('getMatterAccessoryInfo', { uuid })

      const response = await this.homebridgeIpcService.requestResponse(
        'getMatterAccessoryInfo',
        'matterAccessoryInfoData',
      ) as MatterAccessoryInfo
      ...
      if (parts[1]) {
        const part = response.parts?.find((p: MatterAccessoryPart) => p.id === parts[1])
        if (part) {
          return this.transformMatterAccessory(response, part)
        }
        throw new BadRequestException(`Part '${parts[1]}' not found in accessory`)
      }
  ...
  public async saveAccessoryLayout(user: string, layout: Record<string, unknown>) {
    let accessoryLayout: Record<string, unknown>[]
  ...
  private async loadMatterAccessories(): Promise<MatterService[]> {
    ...
    try {
      // Request Matter accessories via IPC and wait for response
  ...
  private handleMatterStateUpdate(data: MatterStateUpdate): void {
    // Find the Matter service
    const uniqueId = data.partId
      ? `matter:${data.uuid}:${data.partId}`
      : `matter:${data.uuid}`
  ...
  private async handleMatterControl(client: Socket, control: {
    ...
    try {
      // Parse uniqueId: matter:{uuid} or matter:{uuid}:{partId}
      const parts = control.uniqueId.replace('matter:', '').split(':')
      const uuid = parts[0]
      ...
      // Wait for response
      const response = await this.homebridgeIpcService.requestResponse(
        'matterAccessoryControl',
        'matterAccessoryControlResponse',
      ) as MatterControlResponse
  ...
  /**
   * Control a Matter accessory (HTTP API)
   */
  public async controlMatterAccessory(
    uuid: string,
    cluster: string,
    attributes: Record<string, unknown>,
    ...
    try {
      // Wait for response using the unified matterEvent
      ...
      if (!response.success) {
        throw new Error(response.error || 'Matter control failed')
      }
      ...
      return {
        success: false,
        error: error.message || 'Matter control failed',
      }
    }
  }
}
