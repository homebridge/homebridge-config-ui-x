...
import { UpdateAllExclusionReason, UpdateAllJournal, UpdateAllPlan, UpdateAllPlanItem } from './update-all.interfaces.js'
...
export class UpdateAllService {
  ...
  public async computePlan(): Promise<UpdateAllPlan> {
    ...
    // problems, but the rest of the plan is still worth returning.
    const homebridgePkg = await this.getPackageSafely(() => this.pluginsService.getHomebridgePackage(), 'Homebridge')
    const uiPkg = await this.getPackageSafely(() => this.pluginsService.getHomebridgeUiPackage(), 'Homebridge UI')
    ...
    for (const plugin of outOfDatePlugins) {
      ...
      this.placeItem(plan, {
        ...
        childBridgeUsernames: reason === null ? await this.pluginsService.getPluginChildBridgeUsernames(plugin.name) : [],
      }, reason)
    }
  ...
  public async start(confirmed: { name: string, to: string }[]): Promise<{ runId: string }> {
    ...
    try {
      ...
      const runId = await this.journalService.startRun(items.map(x => ({ type: x.type, name: x.name, displayName: x.displayName, icon: x.icon, childBridgeUsernames: x.childBridgeUsernames, from: x.from, to: x.to })))
  ...
  private async finale(needs: RestartNeeds): Promise<UpdateAllJournal['restart']> {
    ...
    if (needs.homebridge) {
      ...
      try {
        ...
        restart.childBridges = 'done'
      } catch (e) {
        this.logger.error(`Update All: failed to restart a child bridge (${e.message}).`)
        restart.childBridges = 'failed'
      }
    }
