...
export class UpdateAllController {
  ...
  @ApiOperation({ summary: 'Mark the most recent Update All run as seen, so its summary is shown only once.' })
  @Post('journal/ack')
  async acknowledgeJournal() {
    await this.journalService.acknowledge()
    return { ok: true }
  }
}
