...
describe('UpdateAllController (e2e)', () => {
  ...
  it('a run round-trips through the journal and survives an app restart in spirit', async () => {
    ...
    expect(journal.acknowledged).toBe(false)
  ...
  it('POST /update-all/journal/ack marks the run seen, and is idempotent', async () => {
    await journalService.startRun([{ type: 'homebridge', name: 'homebridge', from: '2.3.1', to: '2.4.0' }])
    ...
    expect((await journalService.read()).acknowledged).toBe(true)
  })
  ...
  it('a journal write against a corrupt file is a no-op, not a crash', async () => {
    ...
    await journalService.acknowledge()
