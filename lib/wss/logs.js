...
module.exports = (ws, req) => {
  if (hb.log && typeof (hb.log) === 'string' && fs.existsSync(hb.log)) {
    ...
    ws.send(JSON.stringify({log: color.cyan(`Loading logs from file\r\nCMD: ${command.join(' ')}\r\n\r\n`)}))
    ...
    let command = Array.isArray(hb.log.tail) ? hb.log.tail : hb.log.tail.split(' ')
    tailLog(ws, req, command)
  } else {
    if (hb.log) {
      ws.send(JSON.stringify({log: color.red(`Log file does not exist: ${hb.log}\r\n`)}))
      ws.send(JSON.stringify({log: color.red(`Please set the correct path to the logs in your Homebridge config.json file.\r\n\r\n`)}))
    } else {
      ws.send(JSON.stringify({log: color.red(`Cannot show logs. Log option is not configured in your Homebridge config.json file.\r\n\r\n`)}))
    }
