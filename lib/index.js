...
class HomebridgeConfigUi {
  constructor (log, config) {
    ...
    hb.storagePath = homebridge.user.storagePath()
    hb.port = config.port || 8080
    hb.authMethod = config.auth
    hb.log = config.log
    ...
    if (config.temp && fs.existsSync(config.temp)) {
      ...
      setTimeout(() => {
        hb.logger(color.yellow(`WARNING: Configured path to temp file does not exist: ${config.temp}`))
        hb.logger(color.yellow(`WARNING: CPU Temp will not be displayed`))
      }, 2000)
    }
