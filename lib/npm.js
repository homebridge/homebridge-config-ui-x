(function () {
  ...
  function declare (moduleName, exports) {
  ...
  var npm = {
    ...
    search: function (search, callback) {
      ...
      if (base && search && search != '') {
        ...
        request({
          url: 'https://api.npms.io/v2/search?q=' + (!search || search.length === 0 ? '' : search + '+') + 'keywords:homebridge-plugin+not:deprecated+not:insecure&size=250',
          json: true
        }, function (err, res, body) {
          var pkgs = []
    ...
    install: function (pkg, callback) {
      ...
      require('child_process').exec('"' + command + '" install -g ' + pkg, function (err, stdout, stderr) {
    ...
    uninstall: function (pkg, callback) {
      ...
      require('child_process').exec('"' + command + '" uninstall -g ' + pkg, function (err, stdout, stderr) {
        callback(err, stdout, stderr)
      })
    },
