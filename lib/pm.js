  ...
  installPlugin (pkg) {
    return this.getInstalled()
      .then(plugins => {
        ...
        return this.executeCommand(`npm install --unsafe-perm ${installOptions} ${pkg}@latest`, installPath)
      })
      .catch((err) => {
  ...
  updateHomebridge () {
    ...
    return childProcess.execAsync('yarn global dir', {cwd: os.tmpdir()})
      ...
      .map(installPath => {
        ...
        return this.executeCommand(`npm install --unsafe-perm homebridge@latest`, installPath)
          .then(() => {
